...
def andes(terms, matrix, g1_term2index, g2_term2index, g1_population, g2_population, ite=1000, distinct=False):
    """
    Parameters
    ----------
    terms : tuple
    ...
    true_score = best_match_average(matrix[np.ix_(indexes1, indexes2)])
    ...
    for i in range(ite):
        ...
        back_scores[i] = best_match_average(back_matrix)
...
def best_match_ranked_list(matrix):
    ...
    calculate the max deivation of running sum
    """
...
def gsea_andes(term, ranked_list, matrix, term2indices, annotated_indices, ite=1000):
    ...
    term_indices = list(term2indices[term])
    
    true_score = best_match_ranked_list(matrix[np.ix_(term_indices, ranked_list)])
    ...
    for i in range(ite):
        ...
        back_s = best_match_ranked_list(back_matrix)
